...
httpRequest.interceptors.response.use(
  ...
  async (error: AxiosError<ApiResponse>) => {
    ...
    // 处理 FastAPI 422 验证错误（无 code 字段，只有 detail）
    if (status === 422 && data !== undefined && data !== null && typeof data === "object") {
      // FastAPI 默认格式: { detail: [{ loc, msg, type }] }
      if ("detail" in data && Array.isArray((data as any).detail)) {
        const details = (data as any).detail as Array<{ loc?: string[]; msg?: string }>;
        const messages = details
          .map((d) => d.msg || "")
          .filter(Boolean)
          .slice(0, 3); // 最多显示3条
        const errorMsg = messages.length > 0 ? messages.join("；") : "请求参数验证失败";
