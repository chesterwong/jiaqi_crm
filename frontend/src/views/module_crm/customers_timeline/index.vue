<!-- 动态时间线：PageSearch + PageContent + useCrudList（与项目 CURD 规范一致）；页面路由 /module_crm/customers_timeline -->
<template>
    ...
    >
      <template #toolbar="{ toolbarRight, onToolbar, removeIds, cols }">
        <CrudToolbarLeft
          ...
          :perm-create="['module_crm:customers_timeline:create']"
          :perm-delete="['module_crm:customers_timeline:delete']"
          :perm-patch="['module_crm:customers_timeline:patch']"
      ...
      <template #table="{ data, loading, tableRef, onSelectionChange, pagination }">
        <div class="data-table__content">
          ...
          >
            ...
            />
            <el-table-column
              v-if="contentCols.find((col) => col.prop === 'event_type')?.show"
              label="事件类型"
              prop="event_type"
              min-width="140"
              show-overflow-tooltip
            />
            <el-table-column
              v-if="contentCols.find((col) => col.prop === 'event_category')?.show"
              label="事件分类"
              prop="event_category"
              min-width="140"
              show-overflow-tooltip
            />
            <el-table-column
              v-if="contentCols.find((col) => col.prop === 'title')?.show"
              label="职位"
              prop="title"
              min-width="140"
              show-overflow-tooltip
            ...
            />
            <el-table-column
              v-if="contentCols.find((col) => col.prop === 'related_name')?.show"
              label="关联名称"
              prop="related_name"
              min-width="140"
              show-overflow-tooltip
            ...
            />
            <el-table-column
              v-if="contentCols.find((col) => col.prop === 'extra_data')?.show"
              label="扩展数据"
              prop="extra_data"
              min-width="140"
              show-overflow-tooltip
            ...
            >
              <template #default="scope">
                <el-button
                  v-hasPerm="['module_crm:customers_timeline:detail']"
                  type="info"
                  size="small"
                  link
                  icon="View"
                ...
                <el-button
                  v-hasPerm="['module_crm:customers_timeline:update']"
                  type="primary"
                  size="small"
                  link
                  icon="Edit"
                ...
                <el-button
                  v-hasPerm="['module_crm:customers_timeline:delete']"
                  type="danger"
                  size="small"
                  link
                  icon="Delete"
    ...
    >
      <template v-if="dialogVisible.type === 'detail'">
        <el-descriptions :column="4" border>
          ...
          <el-descriptions-item label="事件类型" :span="2">
            {{ detailFormData.event_type }}
          </el-descriptions-item>
          <el-descriptions-item label="事件分类" :span="2">
            {{ detailFormData.event_category }}
          </el-descriptions-item>
          ...
          <el-descriptions-item label="关联名称" :span="2">
            {{ detailFormData.related_name }}
          </el-descriptions-item>
      ...
      <template v-else>
        ...
        >
          ...
          <el-form-item label="事件分类" prop="event_category" :required="false">
            <el-input v-model="formData.event_category" placeholder="请输入事件分类" />
          </el-form-item>
          <el-form-item label="职位" prop="title" :required="true">
          ...
          <el-form-item label="关联名称" prop="related_name" :required="false">
            <el-input v-model="formData.related_name" placeholder="请输入关联名称" />
          </el-form-item>
          ...
          <el-form-item label="扩展数据" prop="extra_data" :required="false">
            <el-input v-model="formData.extra_data" placeholder="请输入扩展数据" />
          </el-form-item>
...
import CrmCustomerTimelineAPI, {
  CrmCustomerTimelinePageQuery,
  CrmCustomerTimelineTable,
  CrmCustomerTimelineForm,
} from "@/api/module_crm/customers_timeline";
...
const searchConfig = reactive<ISearchConfig>({
  permPrefix: "module_crm:customers_timeline",
  colon: true,
  isExpandable: true,
  showNumber: 2,
  ...
  formItems: [
    ...
    },
    {
      ...
      label: "事件类型",
      type: "input",
      attrs: { placeholder: "请输入事件类型", clearable: true },
    },
    {
      prop: "event_category",
      label: "事件分类",
      type: "input",
      attrs: { placeholder: "请输入事件分类", clearable: true },
    },
    {
      prop: "title",
      label: "职位",
    ...
    },
    {
      ...
      label: "关联名称",
      type: "input",
      attrs: { placeholder: "请输入关联名称", clearable: true },
    },
    {
      prop: "operator_id",
      label: "操作人ID",
    ...
    },
    {
      ...
      label: "扩展数据",
      type: "input",
      attrs: { placeholder: "请输入扩展数据", clearable: true },
    },
    {
      prop: "created_time",
      label: "创建时间",
...
function normalizeCrmCustomerTimelineQuery(params: Record<string, unknown>) {
  const p = { ...params } as Record<string, unknown>;
  if (Array.isArray(p.created_time) && p.created_time.length === 0) p.created_time = undefined;
  if (Array.isArray(p.updated_time) && p.updated_time.length === 0) p.updated_time = undefined;
  return p as unknown as CrmCustomerTimelinePageQuery;
}

const contentConfig = reactive<IContentConfig<CrmCustomerTimelinePageQuery>>({
  permPrefix: "module_crm:customers_timeline",
  ...
  indexAction: async (params) => {
    const res = await CrmCustomerTimelineAPI.listCrmCustomerTimeline(
      normalizeCrmCustomerTimelineQuery(params as unknown as Record<string, unknown>)
    );
    return {
  ...
  deleteAction: (ids) =>
    CrmCustomerTimelineAPI.deleteCrmCustomerTimeline(
      ids
        .split(",")
        .map((s) => Number(s.trim()))
        .filter((n) => !Number.isNaN(n) && n > 0)
    ),
  importTemplate: () => CrmCustomerTimelineAPI.downloadTemplateCrmCustomerTimeline(),
  importAction: (file: File) => {
    const fd = new FormData();
    fd.append("file", file);
    return CrmCustomerTimelineAPI.importCrmCustomerTimeline(fd).then((res) => {
      if (res.data.code !== ResultEnum.SUCCESS) {
        return Promise.reject(new Error(res.data.msg));
      }
    });
  },
  exportsAction: async (params: CrmCustomerTimelinePageQuery) => {
    const query: Record<string, unknown> = { ...params };
    return fetchAllPages<CrmCustomerTimelineTable>({
      pageSize: 9999,
      initialQuery: query,
      fetchPage: async (q) => {
        const res = await CrmCustomerTimelineAPI.listCrmCustomerTimeline(
          normalizeCrmCustomerTimelineQuery(q as unknown as Record<string, unknown>)
        );
        return {
...
const detailFormData = ref<CrmCustomerTimelineTable>({});

const formData = reactive<CrmCustomerTimelineForm>({
...
const rules = reactive({
  ...
  event_type: [{ required: true, message: "请填写事件类型", trigger: "blur" }],
  event_category: [{ required: false, message: "请填写事件分类", trigger: "blur" }],
  title: [{ required: true, message: "请填写职位", trigger: "blur" }],
  ...
  related_name: [{ required: false, message: "请填写关联名称", trigger: "blur" }],
  ...
  extra_data: [{ required: false, message: "请填写扩展数据", trigger: "blur" }],
...
const initialFormData: CrmCustomerTimelineForm = {
  customer_id: undefined,
  event_type: undefined,
  event_category: undefined,
  title: undefined,
...
async function handleOpenDialog(type: "create" | "update" | "detail", id?: number) {
  ...
  if (id) {
    const response = await CrmCustomerTimelineAPI.detailCrmCustomerTimeline(id);
    if (type === "detail") {
...
async function handleSubmit() {
  dataFormRef.value.validate(async (valid: boolean) => {
    if (valid) {
      ...
      try {
        if (id) {
          await CrmCustomerTimelineAPI.updateCrmCustomerTimeline(id, { ...submitData, [PK]: id });
        } else {
          await CrmCustomerTimelineAPI.createCrmCustomerTimeline(submitData);
        }
        dialogVisible.visible = false;
        await resetForm();
        refreshList();
...
async function handleMoreClick(status: string) {
  ...
  ElMessageBox.confirm(`确认${status === "0" ? "启用" : "停用"}该项数据?`, "警告", {
    ...
    .then(async () => {
      try {
        await CrmCustomerTimelineAPI.batchCrmCustomerTimeline({ ids, status });
        refreshList();
      } catch (error: unknown) {
        console.error(error);
      }
