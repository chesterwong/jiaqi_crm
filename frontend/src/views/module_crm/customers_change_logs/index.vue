<!-- 客户变更日志：PageSearch + PageContent + useCrudList（与项目 CURD 规范一致）；页面路由 /module_crm/customers_change_logs -->
<template>
    ...
    >
      <template #toolbar="{ toolbarRight, onToolbar, removeIds, cols }">
        <CrudToolbarLeft
          ...
          :perm-create="['module_crm:customers_change_logs:create']"
          :perm-delete="['module_crm:customers_change_logs:delete']"
          :perm-patch="['module_crm:customers_change_logs:patch']"
        ...
        <div v-loading="loading" class="change-log-timeline">
          ...
          <el-timeline v-else>
            ...
            >
              ...
              <div v-if="item.change_reason" class="timeline-item-reason">
    ...
    >
      ...
      <template v-else>
        ...
        >
          ...
          <el-form-item label="变更原因" prop="change_reason" :required="false">
            <el-input v-model="formData.change_reason" placeholder="请输入变更原因" />
          </el-form-item>
...
import CrmCustomerChangeLogsAPI, {
  CrmCustomerChangeLogsPageQuery,
  CrmCustomerChangeLogsTable,
  CrmCustomerChangeLogsForm,
} from "@/api/module_crm/customers_change_logs";
...
const searchConfig = reactive<ISearchConfig>({
  permPrefix: "module_crm:customers_change_logs",
  colon: true,
  isExpandable: true,
  showNumber: 2,
  ...
  formItems: [
    ...
    },
    {
      ...
      label: "变更原因",
      type: "input",
      attrs: { placeholder: "请输入变更原因", clearable: true },
    },
    {
      prop: "changed_at",
      label: "变更时间",
...
>([
  ...
  { prop: "change_reason", label: "变更原因", show: true },
...
function normalizeCrmCustomerChangeLogsQuery(params: Record<string, unknown>) {
  const p = { ...params } as Record<string, unknown>;
  if (Array.isArray(p.created_time) && p.created_time.length === 0) p.created_time = undefined;
  if (Array.isArray(p.updated_time) && p.updated_time.length === 0) p.updated_time = undefined;
  return p as unknown as CrmCustomerChangeLogsPageQuery;
}

const contentConfig = reactive<IContentConfig<CrmCustomerChangeLogsPageQuery>>({
  permPrefix: "module_crm:customers_change_logs",
  ...
  indexAction: async (params) => {
    const res = await CrmCustomerChangeLogsAPI.listCrmCustomerChangeLogs(
      normalizeCrmCustomerChangeLogsQuery(params as unknown as Record<string, unknown>)
    );
    return {
  ...
  deleteAction: (ids) =>
    CrmCustomerChangeLogsAPI.deleteCrmCustomerChangeLogs(
      ids
        .split(",")
        .map((s) => Number(s.trim()))
        .filter((n) => !Number.isNaN(n) && n > 0)
    ),
  importTemplate: () => CrmCustomerChangeLogsAPI.downloadTemplateCrmCustomerChangeLogs(),
  importAction: (file: File) => {
    const fd = new FormData();
    fd.append("file", file);
    return CrmCustomerChangeLogsAPI.importCrmCustomerChangeLogs(fd).then((res) => {
      if (res.data.code !== ResultEnum.SUCCESS) {
        return Promise.reject(new Error(res.data.msg));
      }
    });
  },
  exportsAction: async (params: CrmCustomerChangeLogsPageQuery) => {
    const query: Record<string, unknown> = { ...params };
    return fetchAllPages<CrmCustomerChangeLogsTable>({
      pageSize: 9999,
      initialQuery: query,
      fetchPage: async (q) => {
        const res = await CrmCustomerChangeLogsAPI.listCrmCustomerChangeLogs(
          normalizeCrmCustomerChangeLogsQuery(q as unknown as Record<string, unknown>)
        );
        return {
...
const detailFormData = ref<CrmCustomerChangeLogsTable>({});

const formData = reactive<CrmCustomerChangeLogsForm>({
...
const rules = reactive({
  ...
  change_reason: [{ required: false, message: "请填写变更原因", trigger: "blur" }],
...
const initialFormData: CrmCustomerChangeLogsForm = {
  customer_id: undefined,
  field_name: undefined,
  field_label: undefined,
  old_value: undefined,
...
async function handleOpenDialog(type: "create" | "update" | "detail", id?: number) {
  ...
  if (id) {
    const response = await CrmCustomerChangeLogsAPI.detailCrmCustomerChangeLogs(id);
    if (type === "detail") {
...
async function handleSubmit() {
  dataFormRef.value.validate(async (valid: boolean) => {
    if (valid) {
      ...
      try {
        if (id) {
          await CrmCustomerChangeLogsAPI.updateCrmCustomerChangeLogs(id, {
            ...submitData,
            [PK]: id,
          });
        } else {
          await CrmCustomerChangeLogsAPI.createCrmCustomerChangeLogs(submitData);
        }
        dialogVisible.visible = false;
        await resetForm();
        refreshList();
...
async function handleMoreClick(status: string) {
  ...
  ElMessageBox.confirm(`确认${status === "0" ? "启用" : "停用"}该项数据?`, "警告", {
    ...
    .then(async () => {
      try {
        await CrmCustomerChangeLogsAPI.batchCrmCustomerChangeLogs({ ids, status });
        refreshList();
      } catch (error: unknown) {
        console.error(error);
      }
